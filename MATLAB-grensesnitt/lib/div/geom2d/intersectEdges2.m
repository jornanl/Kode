function point = intersectEdges2(edge2, edge)
...
out1 = find(~onEdge(point, edge));
point(out1, :) = repmat([NaN NaN], [length(out1) 1]);

out2 = find(~onEdge(point, edge2));
point(out2, :) = repmat([NaN NaN], [length(out2) 1]);
