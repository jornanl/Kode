...
function [nu,R,H,associations,matches] = matchnnsf(Gpred,Hpred,L,params,displaying)
...
%% Added by bjørn S  
  ...
  for i = 1:nassoc
    ...
    if( associations(i).accept == 1 )
      ...
      j=0;      
      for j=1:nG
          if( associations(i).ig == get( Xg{1,j},'id' ) )
%              disp(j);
              break;
          end;
      end;

      matches(myCounter,1)=j;
      matches(myCounter,2)=get( Xl{1,associations(i).il},'id' ) ;     
        
    end;
  end;
%%End Added by bjørn S  
...
%    end; disp([' ',str1,' matched ',str2]);
  ...
  if displaying,
    figure(4); clf; hold on; zoom on; axis equal;
