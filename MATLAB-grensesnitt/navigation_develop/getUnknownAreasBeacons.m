function cT=getUnknownAreasBeacons(G,myHandles,robotPath)
...
% load all beacons from global map
for i=2:length(Xg)
    beacons(i-1,1:2)=get(Xg{i},'x');
end


rad=str2double(get(myHandles.maxPerceptRadius,'String')); %radius = handle to maxPerceptionRadius
%rad=0.6;
res=str2double(get(myHandles.angularRes,'String')); %
%res=3;
NOP=360/res;
...
if all(pose(k,1:2)), % both endpoints of segment is inside circle
    ...
    elseif phiSegs(2)-phiSegs(1)>pi,
        ...
        warning 'program error.....' %#ok<WNTAG>
    end
...
elseif any(pose(k,1:2)), % one endpoint of segment is inside circle
    ...
    elseif pose(k,2),
        ...
        warning 'program error... UnknownAreas1' %#ok<WNTAG>
    end
