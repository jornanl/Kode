function [error,navData,handles,scndata]=leftWallFollowerToBackTrackerBeacons(G,myCon,navData,gaps,handles)
global RUNNING;
...
elseif ( navData.state == 1)
    ...
    if( navData.state == 1) % did not change state
        ...
        turnRads=quasiFollowLeftWallNavigation2( G );
...
elseif( navData.state == 2 )        
    ...
    if( navData.state == 2 )
        ...
        if(relturn == -1)
            ...
            robAngle = char( tmp );
            [error,scndata]=setRobotTarget(myCon,robAngle, round(min((gotoDist*100)*0.5,20)),handles);
            navData.lastHeading=2*double(robAngle);
