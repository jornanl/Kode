function [navDataNextIndex,gotoThroughThisGapX,gotoThroughThisGapY]...
    =findNextInterestingGapPathIndex3(myNavData,myNavDataAll,gaps)
...
closestInterestingGapPathIndex2 = 0;
finalGapPoints=[];
if( size(interrestingGaps,1) > 0 )
    for m=1:1:size(interrestingGaps,1)
        ...
        if index~=closestInterestingGapPathIndex,
            closestInterestingGapPathIndex2=max(closestInterestingGapPathIndex2,index);
        end
    end;    
end;
...
navDataNextIndex = closestInterestingGapPathIndex2;
if(closestInterestingGapPathIndex== 0)
    return;
end;
