%SIMROBOT Constructor function for differential drive simrobot object.
%   R = SIMROBOT
...
%
%   R = SIMROBOT(NAME,X,C,RL,RR,B,FORMTYPE,T)
...
%
%   The differential drive simrobot class is a child class of the
%   base class robot.
%
%   Examples:
%      r = robotdd('Polly',[1,2,pi]',eye(3),.2,.2,.5,4,now);
%
...
function r = initsimrobot(varargin)
...
switch nargin,
  case 0,
    ...
    r.rr = 0.08;
    r.b  = 0.4;
    rb = robot('differential drive simrobot','',0,0);
    r  = class(r,'robotdd',rb);      
  case 1,
    % if single argument of class robotdd, return it
    if (isa(varargin{1},'simrobotdd'))
      r = varargin{1}; 
    else
      error('simulation/simrobotdd: Input argument not a differential drive simrobot object')
    end
  case 8,
    % create object using specified values
    x = varargin{2};
    C = varargin{3};
    if (numel(x) == 3) && (size(C) == [3,3]),
      r.x  = x;
      r.C  = C;
      r.rl = varargin{4};
      r.rr = varargin{5};
      r.b  = varargin{6};
      rb = robot('differential drive simrobot',varargin{1},varargin{7},varargin{8});
      r  = class(r,'simrobotdd',rb);
    else
      error('simulation/simrobotdd: Wrong argument syntax');
    end;
  otherwise
    error('simulation/simrobotdd: Wrong number of input arguments')
end
