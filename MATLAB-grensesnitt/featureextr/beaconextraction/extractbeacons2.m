...
function L = extractbeacons2(bcndata,params,displ,pos)
...
%valids=bcndata.steps.data1;
% removing scandata too close or too far away from robot
%tooClose=((xR(:)-0).^2+(yR(:)-0).^2) < CLOSEDIST^2;
%xR(tooClose)=[];
%yR(tooClose)=[];
%beacons= [xR yR];
if  ((xR(1)-0)^2+(yR(1)-0)^2) < CLOSEDIST^2 || ((xR(1)-0)^2+(yR(1)-0)^2) > IGNOREDIST^2,
    beacons = [];
else
    beacons = [xR yR];
end
...
%   end; disp([' ',str1,' ',str2,' extracted']);
  ...
  if displ,
    figure(4); %clf;
    hold on; set(gca,'Box','on'); axis equal;
