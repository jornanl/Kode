...
while(~StopSensorWatchLoop)
    ...
    if PlotData
        if ~USmode            
            set(handles.txtPeriodCount, 'String', out.ScaledVal);
            set(handles.txtCurVal, 'String', out.NormalizedADVal);
            plot([toctoc(3) toctoc(3)], [0 out.NormalizedADVal], 'LineWidth', LineWidth, 'Color', cmap(out.NormalizedADVal + 1, :))
        else
            set(handles.txtCurVal, 'String', out);
            plot([toctoc(3) toctoc(3)], [0 out], 'LineWidth', LineWidth, 'Color', cmap(out + 1, :))
        end%if
...
try
    ...
    if status ~= 0
        ...
        COM_SetDefaultNXT(COM_OpenNXT('bluetooth.ini', 'check'));
    end%if
catch
    COM_CloseNXT('all', 'bluetooth.ini');
    COM_SetDefaultNXT(COM_OpenNXT('bluetooth.ini', 'check'));
